...
【问题】每个属性 setter 中重复 `self.data_changed.emit()` 代码。
【解决方案】使用描述符（Descriptor）封装属性变更通知逻辑。
...
class PositionProperty(ObservableProperty[int]):
    ...
    ## 修复说明 (2026-04-02 MCP架构审查)
    专门处理位置属性，同时触发 data_changed 和 position_changed。
    """
...
class SizeProperty(ObservableProperty[int]):
    ...
    ## 修复说明 (2026-04-02 MCP架构审查)
    专门处理大小属性，同时触发 data_changed 和 size_changed。
    """
...
class SignalBlocker:
    ...
    使用示例:
        with SignalBlocker(model):
            model.x = 100
            model.y = 200
            model.width = 300
        # 退出上下文后自动触发一次 data_changed
    """
