...
【问题】emit_signal 每次遍历所有连接，O(N)复杂度，连接数量多时性能差。

【解决方案】建立信号-连接索引，复杂度降为 O(1) 查找 + O(M) 执行（M为匹配连接数）。

【性能对比】
- 优化前：1000个连接，每次信号发射遍历1000次
- 优化后：直接定位到匹配的连接，只遍历实际需要的连接
...
class IndexedCommunicationManager(QObject):
    ...
    ## 修复说明 (2026-04-02 MCP三轮审查)
    使用双重索引优化信号查找性能：
    1. _signal_index: (source_id, signal_type) -> [conn_id, ...]
    2. _target_index: target_id -> [conn_id, ...]
