...
注意:
    - 不直接访问 CommunicationManager._connections 私有属性
    - 使用 CommunicationManager 的公开方法（get_all_connections, get_connection 等）
    - SignalType/CommActionType 枚举值在 ComboBox 中存储为字符串（.value）
"""
...
from models import (
    SignalType, CommActionType, SignalConnectionSchema,
...
class SignalEditDialog(QDialog):
    ...
    def __init__(self, components: List[Dict], connection: Optional[SignalConnectionSchema] = None, 
    ...
    def _setup_ui(self):
        ...
        for sig in SignalType:
            self._signal_combo.addItem(sig.value, sig.value)
        ...
        for action in CommActionType:
            self._action_combo.addItem(action.value, action.value)
    ...
    def _load_connection(self, conn: SignalConnectionSchema):
        """加载现有连接数据。"""
    ...
    def _on_accept(self):
        ...
        if not source_id:
            ...
            WarningDialog.show_warning(self, "提示", "请选择源组件")
            return
        
        if not target_id:
            from views.custom_dialogs import WarningDialog
            WarningDialog.show_warning(self, "提示", "请选择目标组件")
            return
...
class SignalManagerPanel(QWidget):
    ...
    def refresh(self):
        ...
        source_groups: Dict[str, List[SignalConnectionSchema]] = {}
        
        for conn in self._comm_manager.get_all_connections():
    ...
    def _on_add_signal(self):
        ...
        if not self._components:
            from views.custom_dialogs import WarningDialog
            WarningDialog.show_warning(self, "提示", "请先添加组件")
            return
    ...
    def _on_edit_signal(self):
        ...
        conn = self._comm_manager.get_connection(conn_id)
        if not conn:
            return
    ...
    def get_all_connections(self) -> List[SignalConnectionSchema]:
        return self._comm_manager.get_all_connections()
