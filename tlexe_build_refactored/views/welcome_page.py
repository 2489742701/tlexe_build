...
class WelcomePage(QWidget):
    ...
    def __init__(self):
        ...
        _log.info("WelcomePage.__init__ start")
        ...
        self._init_ui()
        _log.info("_init_ui done")
        
        self._update_greeting()
        _log.info("_update_greeting done")
        ...
        QTimer.singleShot(0, self._update_unsaved_projects)
        QTimer.singleShot(0, self._update_recent_projects)
    ...
    def _init_ui(self):
        ...
        _log.info("_init_ui: layout created")
        ...
        _log.info("_init_ui: buttons done")
        ...
        _log.info("_init_ui: scroll_area created")
        
        scroll_content = QWidget()
        scroll_layout = QVBoxLayout(scroll_content)
        scroll_layout.setContentsMargins(0, 0, 0, 0)
        scroll_layout.setSpacing(15)
        _log.info("_init_ui: scroll_content created")
        
        _log.info("_init_ui: creating unsaved_section...")
        unsaved_section = CollapsibleSection("未保存项目", expanded=True)
        _log.info("_init_ui: unsaved_section created")
        self._unsaved_list = unsaved_section.content_widget
        scroll_layout.addWidget(unsaved_section)
        _log.info("_init_ui: unsaved_section done")
        ...
        _log.info("_init_ui: examples_section done")
        ...
        _log.info("_init_ui: recent_section done")
        
        scroll_layout.addStretch()
        scroll_area.setWidget(scroll_content)
        layout.addWidget(scroll_area)
        _log.info("_init_ui: COMPLETE")
        ...
        try:
            if template_path == "templates/test_template.py":
                ...
                if os.path.exists(full_path):
                    ...
                    WarningDialog.show_warning(self, "文件不存在", f"示例文件不存在: {full_path}")
            else:
            ...
            ErrorDialog.show_error(self, "打开案例失败", f"无法打开案例:\n{str(e)}\n\n错误日志已保存到:\n{crash_file}")
    ...
    def _on_unsaved_project_clicked(self, file_path: str):
        ...
        if os.path.exists(file_path):
            ...
            from views.custom_dialogs import WarningDialog
            WarningDialog.show_warning(self, "文件不存在", "未保存项目文件已不存在")
    ...
    def _on_delete_project_file(self, file_path: str):
        ...
        try:
            ...
            from views.custom_dialogs import WarningDialog
            WarningDialog.show_warning(self, "删除失败", f"无法删除文件:\n{e}")
            return
    ...
    def _on_recent_project_clicked(self, file_path: str):
        ...
        if os.path.exists(file_path):
            ...
            from views.custom_dialogs import WarningDialog
            WarningDialog.show_warning(self, "文件不存在", f"项目文件不存在: {file_path}")
...
class CollapsibleSection(QWidget):
    ...
    def _init_ui(self):
        ...
        import logging
        _log = logging.getLogger(__name__)
        _log.info(f"CollapsibleSection._init_ui start: {self._title}")
        
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(0)
        _log.info(f"CollapsibleSection._init_ui layout done: {self._title}")
        
        header = QPushButton(f"{'▼' if self._expanded else '▶'} {self._title}")
        _log.info(f"CollapsibleSection._init_ui header created: {self._title}")
