...
class AlternatingPropertyEditor(BasePropertyEditor):
    ...
    核心：动画时长 + 开始/停止按钮绑定 + 候选项列表管理。
    """

    buttons_set = Signal(str, str, str)
    ...
    def _on_bind_button(self, key):
        ...
        if key == 'start' and getattr(self._model, 'is_running', False):
            QMessageBox.warning(self, "提示", "交替变换运行中，无法设定按钮")
            return
    ...
    def _update_ui_from_model(self):
        ...
        self._update_items_list()
        self.add_btn.setText("添加图片" if getattr(self._model, 'display_mode', 'text') == 'image' else "添加文字")
